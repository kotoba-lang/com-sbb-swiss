(ns sbb_swiss.main
  "Kotodama WASM entrypoint for the sbb_swiss clean-room actor (L4) — Clojure port.
  ...
  facts (`sbb_swiss.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sbb_swiss")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sbb_swiss.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sbb_swiss-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sbb_swiss-compat/ready)
